...
namespace BreganTwitchBot.TwitchCommands.FollowAge
{
    class UserFollowAge
    {
        ...
        public static string GetFollowSince(string username)
        {
            try
            {
                var getUserID = TwitchApiConnection.ApiClient.V5.Users.GetUserByNameAsync(username).Result.Matches;
                ...
                return $"@{username} => You have been following {StartService.ChannelName} since {followTime}";
            }
            catch (Exception exception)
            {
                Log.Fatal(exception.Message);
        ...
        public static string GetOtherUserFollowSince(string usernameRequester, string username)
        {
            try
            {
                var getUserID = TwitchApiConnection.ApiClient.V5.Users.GetUserByNameAsync(username).Result.Matches;
                ...
                var followTime = checkFollow.Follows[0].FollowedAt;
                return $"@{usernameRequester} => {username} has been following {StartService.ChannelName} since {followTime}";
            }
